...
const filterByStatus = () => {
    router.get(route('vouchers.index'), { status: statusFilter.value }, { preserveState: true, replace: true });
};

const deleteVoucher = (id) => { if (confirm('Hapus voucher ini?')) router.delete(route('vouchers.destroy', id)); };
...
const printVoucher = (id) => window.open(route('vouchers.print', id), '_blank');
...
const printSelectedVouchers = () => {
    ...
    const url = new URL(route('vouchers.print.batch'));
    ...
    <AuthenticatedLayout>
        ...
        <div class="min-h-screen">
            <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 space-y-6">
                ...
                <!-- Empty State -->
                <EmptyState v-if="vouchers.data.length === 0 && !statusFilter" icon="ticket" title="Belum Ada Voucher" description="Generate voucher belanja pertama Anda." :action-url="route('vouchers.create')" action-text="Generate Voucher" />
