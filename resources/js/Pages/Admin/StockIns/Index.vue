...
watch(filters, (newFilters) => {
    ...
    router.get(route('stock-ins.index'), query, {
        preserveState: true,
        replace: true,
    });
}, { deep: true });
                                ...
                                <tr v-for="stock in stock_ins.data" :key="stock.id" class="hover:bg-white/5 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end gap-2">
                                            <Link v-if="can('stock_ins.show')" :href="route('stock-ins.show', stock.id)" class="p-2 text-blue-400 hover:text-blue-300 bg-blue-500/10 hover:bg-blue-500/20 rounded-lg transition-colors" title="Detail">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                            </Link>
                                            <Link v-if="can('stock_ins.edit')" :href="route('stock-ins.edit', stock.id)" class="p-2 text-amber-400 hover:text-amber-300 bg-amber-500/10 hover:bg-amber-500/20 rounded-lg transition-colors" title="Edit">
