...
const searchStudent = async () => {
    ...
    try {
        const response = await axios.get(route('kasir.transactions.search.student'), { params: { search: searchQuery.value } });
...
const processTopup = async () => {
    ...
    loading.value = true;
    try {
        const response = await axios.post(route('kasir.transactions.topup'), { student_id: selectedStudent.value.id, amount: topupAmount, description: description.value });
        ...
        <div class="min-h-screen">
            <div class="max-w-2xl mx-auto px-3 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-slate-800/50 backdrop-blur-md border border-white/10 rounded-xl p-4">
                    <Link :href="route('kasir.transactions.index')" class="inline-flex items-center px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-white font-semibold rounded-lg border border-white/10 transition-all text-sm">
                        ← Kembali
                    </Link>
                </div>
                ...
                <div class="bg-slate-800/50 backdrop-blur-md overflow-hidden rounded-xl border border-white/10 shadow-xl p-6">
                    <div class="space-y-6">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            ...
                            <Link :href="route('kasir.transactions.index')" class="inline-flex items-center justify-center px-6 py-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg font-semibold border border-white/10 w-full sm:w-auto">Batal</Link>
                        </div>
                    </div>
                </div>
