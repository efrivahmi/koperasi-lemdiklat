            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
                ...
                <div class="mb-6 bg-gradient-to-r from-purple-900/80 via-indigo-900/80 to-blue-900/80 backdrop-blur-md rounded-xl shadow-2xl p-6 text-white border border-white/10 relative overflow-hidden">
                    ...
                    <div class="relative flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-indigo-200">Tambah Kategori Baru</h3>
                            <p class="text-indigo-200/80 text-sm mt-1">Buat kategori produk untuk organisasi produk koperasi</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-slate-800/50 backdrop-blur-md overflow-hidden shadow-xl rounded-xl border border-white/10">
                    <div class="p-6 sm:p-8">
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div>
                                <label for="unit_group" class="block text-sm font-medium text-slate-300 mb-2">
                                    Kelompok Satuan (Unit Group)
                                </label>
                                <select
                                    id="unit_group"
                                    v-model="form.unit_group"
                                ...
                                >
                                    <option value="" disabled>Pilih Kelompok Satuan (Opsional)</option>
                                    <option v-for="category in UNIT_CATEGORIES" :key="category.id" :value="category.id">
                                        {{ category.name }}
                                    </option>
                                </select>
                                <p class="text-xs text-slate-400 mt-2">Akan otomatis memfilter satuan saat membuat produk di kategori ini.</p>
                                <InputError :message="form.errors.unit_group" class="mt-2" />
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row items-center gap-4 pt-4 border-t border-white/5">
                                ...
                                >
                                    ...
                                    <span v-if="!form.processing">Simpan Kategori</span>
                                    <span v-else>Menyimpan...</span>
                                </button>
                                <Link :href="route('categories.index')" class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-3 bg-slate-700 hover:bg-slate-600 text-white font-medium rounded-lg shadow-sm border border-white/5 transition-all">
