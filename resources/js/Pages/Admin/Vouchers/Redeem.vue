...
const searchStudent = async () => {
    ...
    searchTimeout = setTimeout(async () => {
        try {
            const response = await axios.get(route('vouchers.search.student'), { params: { search: searchQuery.value } });
...
const redeemVoucher = async () => {
    ...
    loading.value = true;
    try {
        const response = await axios.post(route('vouchers.redeem'), { code: voucherCode.value, student_id: selectedStudent.value.id });
...
const autoSelectStudentByNIS = async (nis) => {
    try {
        ...
        const response = await axios.get(route('vouchers.search.student'), { params: { search: nis } });
                ...
                <div class="bg-slate-800/50 backdrop-blur-md overflow-hidden rounded-xl border border-white/10 shadow-xl p-6 sm:p-8">
                    <div class="space-y-6">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            ...
                            <Link :href="route('vouchers.index')" class="inline-flex items-center justify-center px-6 py-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg font-semibold border border-white/10 w-full sm:w-auto">Batal</Link>
                        </div>
                    </div>
                </div>
            </div>
