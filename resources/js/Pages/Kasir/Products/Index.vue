...
const deleteProduct = (product) => {
    if (confirm(`Apakah Anda yakin ingin menghapus produk "${product.name}"?`)) {
        router.delete(route('kasir.products.destroy', product.id));
    }
};
</script>
    ...
    <AuthenticatedLayout>
        ...
        <div class="min-h-screen">
            <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
                ...
                <!-- Toolbar -->
                <div class="relative z-30 mb-6 bg-slate-800/50 backdrop-blur-md border border-white/10 rounded-xl shadow-2xl p-4 space-y-4">
                    <!-- Row 1: Title + Action Buttons -->
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            <h3 class="text-lg font-semibold text-white">Daftar Produk</h3>
                            <p class="text-sm text-slate-400 hidden sm:block">Kelola data produk, harga, dan stok koperasi</p>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            <Link v-if="can('products.create')" :href="route('kasir.products.create')" class="inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white text-sm font-semibold rounded-lg shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 transition-all duration-200">
                                <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
                                Tambah Produk
                            </Link>
                            <Link v-if="can('products.create')" :href="route('kasir.products.barcode-generator')" class="inline-flex items-center justify-center px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm font-semibold rounded-lg border border-white/10 transition-all duration-200">
                    ...
                    <div class="flex gap-2">
                        <div class="relative flex-1">
                            ...
                            <input 
                                ...
                                class="w-full pl-10 pr-4 py-2.5 border border-slate-600 rounded-lg bg-slate-900/70 text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm placeholder-slate-500"
                            >
                        </div>
                        <button 
                        ...
                        <button 
                            v-if="filters.search"
                            @click="search = ''; router.get(route('kasir.products.index'), { sort: sortBy }, { preserveState: true, replace: true })"
                            class="px-3 py-2.5 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-lg transition-colors text-sm shrink-0 border border-white/10"
                    ...
                    <div class="flex items-center gap-2 pt-1 border-t border-white/5">
                        <svg class="w-4 h-4 text-slate-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12"/></svg>
                        <span class="text-sm text-slate-400 shrink-0">Urutkan:</span>
                        <select 
                            v-model="sortBy"
                            class="appearance-none pl-3 pr-8 py-1.5 bg-slate-900/50 border border-white/10 rounded-lg text-white text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 cursor-pointer"
                        >
                                        ...
                                        <td colspan="7" class="px-6 py-4">
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fadeIn pl-16">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="flex flex-wrap gap-2">
                                                        <Link v-if="can('products.show')" :href="route('kasir.products.show', product.id)" class="inline-flex items-center px-3 py-1.5 bg-sky-600 hover:bg-sky-500 text-white text-xs font-medium rounded-md transition-colors shadow-sm ring-1 ring-white/10">
                                                            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                                            Detail
                                                        </Link>
                                                        <Link v-if="can('products.edit')" :href="route('kasir.products.edit', product.id)" class="inline-flex items-center px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-medium rounded-md transition-colors shadow-sm ring-1 ring-white/10">
                                    ...
                                    <td colspan="5" class="px-6 py-12">
                                        ...
                                        >
                                            <template v-if="filters.search" #action>
                                                 <button @click="router.get(route('kasir.products.index'))" class="mt-4 text-indigo-400 hover:text-indigo-300 font-medium text-sm">
                                                    Bersihkan pencarian
                                                </button>
                                            </template>
                                        </EmptyState>
