...
const deleteStudent = (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus siswa ini?')) {
        router.delete(route('kasir.students.destroy', id));
    }
};
...
const printCard = (studentId) => {
    window.open(route('kasir.students.card.generate', studentId), '_blank');
};
...
const printBatchCards = () => {
    ...
    form.method = 'POST';
    form.action = route('kasir.students.cards.batch');
    form.target = '_blank';
        ...
        <div class="py-6 sm:py-12">
            <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
                ...
                <div class="mb-6 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800 border border-purple-200 dark:border-purple-500/30 rounded-lg shadow-sm p-4">
                    <div class="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <Link v-if="can('students.create')" :href="route('kasir.students.create')" class="flex-1 sm:flex-initial inline-flex items-center justify-center px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow-sm transition">
                                <svg class="w-5 h-5 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <EmptyState
                    ...
                    :action-url="route('kasir.students.create')"
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    <tr v-for="student in students.data" :key="student.id" :class="{ 'bg-purple-50 dark:bg-purple-900/20': isSelected(student.id), 'hover:bg-gray-50 dark:hover:bg-gray-700': !isSelected(student.id) }">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            <Link v-if="can('students.edit')" :href="route('kasir.students.edit', student.id)" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 mr-3">Edit</Link>
                                            <Link v-if="!student.rfid_uid && can('students.edit')" :href="route('kasir.students.rfid.register', student.id)" class="text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300 mr-3">RFID</Link>
